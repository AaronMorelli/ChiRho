...
GO
CREATE PROCEDURE [dbo].[sp_XR_FrequentQueries] 
/*   
	...
	PURPOSE: Returns a listing of queries (internally, StmtStoreIDs from [CoreXR].[SQLStmtStore] 
...
exec sp_XR_FrequentQueries @start=''<start datetime>'', @end=''<end datetime>'', @minocc=15, 
	...
	EXEC @@XRDATABASENAME@@.AutoWho.ViewFrequentQueries @init = @lv__CollectionInitiatorID,
			@start = @start, 
			@end = @end, 
			@minocc = @minocc,
			@spids = @spids, 
