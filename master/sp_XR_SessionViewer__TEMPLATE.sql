...
GO
CREATE PROCEDURE [dbo].[sp_XR_SessionViewer]
/*   
	...
	*************************************************************************************************************************************************/
	...
	BEGIN
		...
		We have two different, but related, caching tables. 
			CoreXR.OrdinalCachePosition --> acts a bit like a cursor in the sense that it keeps track of which "position" the user is at currently 
		...
		*/
		...
		--The below code block sets/updates CoreXR.OrdinalCachePosition appropriately.
		...
		BEGIN
			...
			IF NOT EXISTS (
				SELECT * FROM @@XRDATABASENAME@@.CoreXR.OrdinalCachePosition t WITH (NOLOCK)
			...
			BEGIN
				INSERT INTO @@XRDATABASENAME@@.CoreXR.OrdinalCachePosition
			...
			BEGIN	--cache already exists, so someone has already run with this start/endtime before on this spid
				...
				FROM @@XRDATABASENAME@@.CoreXR.OrdinalCachePosition
				...
				BEGIN
					...
					UPDATE @@XRDATABASENAME@@.CoreXR.OrdinalCachePosition
			...
			FROM @@XRDATABASENAME@@.CoreXR.OrdinalCachePosition t
			WHERE t.Utility = @lv__UtilityName 
			AND t.CollectionInitiatorID = @lv__CollectionInitiatorID
			AND t.StartTime = @start
			AND t.EndTime = @end 
...
#Tasks			The number of tasks allocated to a specific running query. The special row "Tasks W/O SPIDs" holds a COUNT(*) from the 
				...
				However, that value can differ from the number of task rows saved to the AutoWho.TasksAndWaits table for parallel queries for 
				2 reasons: 1) Unless a parallel query''s duration exceeds the "ParallelWaitsThreshold" AutoWho option, only its top task/wait 
				is saved to AutoWho.TasksAndWaits. This saves on processing time and disk space, since shorter-lived parallel queries may not 
				...
				dm_os_tasks/dm_os_waiting_tasks are saved to AutoWho.TasksAndWaits, the fact that 2 different DMVs were accessed at 2 different 
