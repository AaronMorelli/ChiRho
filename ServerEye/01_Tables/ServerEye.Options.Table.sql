/*
   ...
   limitations under the License.
	...
	FILE NAME: ServerEye.Options.Table.sql

	TABLE NAME: ServerEye.Options
...
GO
CREATE TABLE [ServerEye].[Options](
...
GO
ALTER TABLE [ServerEye].[Options]  WITH CHECK ADD  CONSTRAINT [CK_SEOptionsForce1Row] CHECK  (([RowID]=(1)))
GO
ALTER TABLE [ServerEye].[Options] CHECK CONSTRAINT [CK_SEOptionsForce1Row]
GO
ALTER TABLE [ServerEye].[Options]  WITH CHECK ADD  CONSTRAINT [CK_SEOptionsServerEyeEnabled] CHECK  (([ServerEyeEnabled]=N'Y' OR [ServerEyeEnabled]=N'N'))
GO
ALTER TABLE [ServerEye].[Options] CHECK CONSTRAINT [CK_SEOptionsServerEyeEnabled]
GO
ALTER TABLE [ServerEye].[Options]  WITH CHECK ADD  CONSTRAINT [CK_SEOptionsBeginEndTime] CHECK  ([BeginTime]<>[EndTime])
GO
ALTER TABLE [ServerEye].[Options] CHECK CONSTRAINT [CK_SEOptionsBeginEndTime]
GO
ALTER TABLE [ServerEye].[Options]  WITH CHECK ADD  CONSTRAINT [CK_SEOptionsBeginEndIsUTC] CHECK  (([BeginEndIsUTC]=N'Y' OR [BeginEndIsUTC]=N'N'))
GO
ALTER TABLE [ServerEye].[Options] CHECK CONSTRAINT [CK_SEOptionsBeginEndIsUTC]
GO
ALTER TABLE [ServerEye].[Options]  WITH CHECK ADD  CONSTRAINT [CK_SEOptionsIntervalLength] CHECK  (([IntervalLength] IN (1, 2, 5)))
GO
ALTER TABLE [ServerEye].[Options] CHECK CONSTRAINT [CK_SEOptionsIntervalLength]
GO
ALTER TABLE [ServerEye].[Options]  WITH CHECK ADD  CONSTRAINT [CK_SEOptionsRetention_Days] CHECK  (([Retention_Days]>=(3)))
GO
ALTER TABLE [ServerEye].[Options] CHECK CONSTRAINT [CK_SEOptionsRetention_Days]
GO
ALTER TABLE [ServerEye].[Options]  WITH CHECK ADD  CONSTRAINT [CK_SEOptionsDebugSpeed] CHECK  (([DebugSpeed]=N'Y' OR [DebugSpeed]=N'N'))
GO
ALTER TABLE [ServerEye].[Options] CHECK CONSTRAINT [CK_SEOptionsDebugSpeed]
GO
ALTER TABLE [ServerEye].[Options]  WITH CHECK ADD  CONSTRAINT [CK_SEOptionsPurgeUnextractedData] CHECK  (([PurgeUnextractedData]=N'N' OR [PurgeUnextractedData]=N'Y'))
GO
ALTER TABLE [ServerEye].[Options] CHECK CONSTRAINT [CK_SEOptionsPurgeUnextractedData]
GO
--
...
GO
EXEC sys.sp_addextendedproperty @name=N'MS_Description', @value=N'The # of days to retain data captured by the ServerEye.Collector procedure.' , @level0type=N'SCHEMA',@level0name=N'ServerEye', @level1type=N'TABLE',@level1name=N'Options', @level2type=N'COLUMN',@level2name=N'Retention_Days'
GO
EXEC sys.sp_addextendedproperty @name=N'MS_Description', @value=N'Whether to capture duration info for each significant statement in the ServerEye.Collector procedure and write that duration info to a table.' , @level0type=N'SCHEMA',@level0name=N'ServerEye', @level1type=N'TABLE',@level1name=N'Options', @level2type=N'COLUMN',@level2name=N'DebugSpeed'
GO
EXEC sys.sp_addextendedproperty @name=N'MS_Description', @value=N'Whether purge is allowed to delete data for capture times rows (in ServerEye.CaptureTimes) that has not been extracted for the DW yet. Takes "Y" or "N"' , @level0type=N'SCHEMA',@level0name=N'ServerEye', @level1type=N'TABLE',@level1name=N'Options', @level2type=N'COLUMN',@level2name=N'PurgeUnextractedData'
GO