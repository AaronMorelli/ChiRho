...
To Execute
------------------------
EXEC AutoWho.PostProcessor @optionset='BackgroundTrace', @init=255, 
...
(
	...
	@startUTC		DATETIME=NULL,	-- AutoWho.CaptureTimes or AutoWho.UserCollectionTimes tables.
...
BEGIN TRY
	...
	BEGIN
		...
		--that these times are valid in the AutoWho.CaptureTimes table
		SET @lv__ExecMode = 2;
	...
	BEGIN
		...
		BEGIN TRY
			EXEC [AutoWho].[CalcBatchStmtCaptureTimes] @FirstCaptureTimeUTC = @startUTC, @LastCaptureTimeUTC = @endUTC;
		END TRY
		BEGIN CATCH
	...
	BEGIN TRY
		EXEC AutoWho.ResolveNodeStatusInfo @CollectionInitiatorID=@init, @FirstCaptureTimeUTC=@startUTC, @LastCaptureTimeUTC=@endUTC;
	END TRY
	BEGIN CATCH
		--We are going to swallow and log the exception, because there's still value in executing the other Post-processing subprocs
		--(they are largely independent of each other). The sub-proc should have logged its failure reason. We just need
	...
	BEGIN
		...
		BEGIN TRY
			EXEC AutoWho.ResolveLockWaits @CollectionInitiatorID=@init, @FirstCaptureTimeUTC=@startUTC, @LastCaptureTimeUTC=@endUTC;
		END TRY
		BEGIN CATCH
	...
	BEGIN
		...
		BEGIN TRY
			EXEC AutoWho.ResolvePageLatches @CollectionInitiatorID=@init, @FirstCaptureTimeUTC=@startUTC, @LastCaptureTimeUTC=@endUTC;
		END TRY
		BEGIN CATCH
			--We are going to swallow the exception, because there's still value in executing the other Post-processing subprocs
			--(they are largely independent of each other). The sub-proc should have logged its failure reason. We just need
