...
GO
CREATE PROCEDURE [AutoWho].[PostProcessor]
/*   
	...
	FILE NAME: AutoWho.PostProcessor.StoredProcedure.sql

	PROCEDURE NAME: AutoWho.PostProcessor
...
BEGIN
	...
	BEGIN
		IF NOT EXISTS (
			SELECT * FROM AutoWho.CaptureTimes ct
			WHERE ct.CollectionInitiatorID = @init 
			AND ct.SPIDCaptureTime = @singletime
			)
		BEGIN
			EXEC sp_releaseapplock @Resource = @lv__AppLockResource, @LockOwner = 'Session';
			RAISERROR('No AutoWho data found for the time specified in @singletime for this @init Collection initiator.', 16, 1);
			RETURN -1;
		END
		...
		SET @end = @singletime;
	END
	ELSE	--@singletime is null, we actually need to validate start and end.
	BEGIN
		IF @start IS NULL OR @end IS NULL
		BEGIN
			EXEC sp_releaseapplock @Resource = @lv__AppLockResource, @LockOwner = 'Session';
			RAISERROR('Parameters @start and @end must be non-null and valid datetime values in the past.', 16, 1);
			RETURN -1;
		END
		...
		SET @end = DATEADD(SECOND, -10, @end);		--to avoid any race conditions with active collection traces (background or user collections)
	...
	FROM (
		SELECT 
			...
			NumMetaLock = SUM(CASE WHEN taw.wait_special_category = @enum__waitspecial__lck AND taw.wait_special_number = 11
								THEN 1 ELSE 0 END)
		FROM AutoWho.TasksAndWaits taw
			INNER JOIN #ToPostProcess t
				ON taw.SPIDCaptureTime = t.SPIDCaptureTime
		WHERE taw.CollectionInitiatorID = @init
		AND taw.SPIDCaptureTime BETWEEN @start AND @end --old BETWEEN: CONVERT(DATETIME,@lv__OverallMinLastTime) AND CONVERT(DATETIME,@lv__CurrentExecTime)
	...
	BEGIN
		...
		*/
		...
		FROM AutoWho.CaptureTimes targ
			INNER JOIN #ToPostProcess t
				ON targ.SPIDCaptureTime = t.SPIDCaptureTime
		WHERE targ.CollectionInitiatorID = @init
		AND targ.SPIDCaptureTime BETWEEN @start AND @end
		;
...
*/
		...
		FROM (
			...
			FROM (
				...
				AND taw.SPIDCaptureTime BETWEEN @start AND @end
					--old BETWEEN: CONVERT(DATETIME,@lv__AutoWhoLastNodeStatusResolve)
	...
	WHERE targ.CollectionInitiatorID = @init
	AND targ.SPIDCaptureTime BETWEEN @start AND @end
							--BETWEEN CONVERT(DATETIME,@lv__AutoWhoLastNodeStatusResolve)
							-- AND CONVERT(DATETIME,@lv__CurrentExecTime)
	...
	*/
	...
	BEGIN
		...
		BEGIN
			...
			--old BETWEEN:
			--AND taw.SPIDCaptureTime BETWEEN CONVERT(DATETIME,@lv__AutoWhoLastLatchResolve) AND CONVERT(DATETIME,@lv__CurrentExecTime)
			...
			*/
			...
			WHERE taw.CollectionInitiatorID = @init
			AND taw.SPIDCaptureTime BETWEEN @start AND @end
			--old BETWEEN: AND taw.SPIDCaptureTime BETWEEN CONVERT(DATETIME,@lv__AutoWhoLastLatchResolve) AND CONVERT(DATETIME,@lv__CurrentExecTime)
			AND taw.wait_special_category IN (@enum__waitspecial__pg, @enum__waitspecial__pgio, @enum__waitspecial__pgblocked)
			;
		...
		*/
		...
		BEGIN
			...
			;WITH extractedData AS (
				...
				WHERE taw.CollectionInitiatorID = @init
				AND taw.SPIDCaptureTime BETWEEN @start AND @end
				--old BETWEEN: AND taw.SPIDCaptureTime BETWEEN CONVERT(DATETIME,@lv__AutoWhoLastLockResolve) AND CONVERT(DATETIME,@lv__CurrentExecTime)
				AND taw.resolution_successful = CONVERT(BIT,0)
