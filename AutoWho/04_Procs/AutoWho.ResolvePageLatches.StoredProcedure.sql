...
GO
CREATE PROCEDURE [AutoWho].[ResolvePageLatches] 
/*   
	...
	FILE NAME: AutoWho.ResolvePageLatches.StoredProcedure.sql

	PROCEDURE NAME: AutoWho.ResolvePageLatches
...
To Execute
------------------------
EXEC AutoWho.ResolvePageLatches @CollectionInitiatorID = 255, @FirstCaptureTimeUTC='2017-07-24 04:00', @LastCaptureTimeUTC='2017-07-24 06:00'
*/
(
...
BEGIN TRY
	...
	IF @rc = 0
	BEGIN
		EXEC AutoWho.LogEvent @ProcID=@@PROCID, @EventCode=0, @TraceID=NULL, @Location=N'No Unprocessed LatchWaits', @Message='No unprocessed LatchWait rows to process';
		RETURN 0;
	END
	...
	BEGIN
		...
		BEGIN
			EXEC AutoWho.LogEvent @ProcID=@@PROCID, @EventCode=0, @TraceID=NULL, @Location=N'No LatchWaits found', @Message='No page latch waits found in this period. Exiting...';
		END
		RETURN 0;
	END
	...
	*/
	...
	SET PostProcessed_Latch = 255
	FROM #LatchWaitProcessCaptureTimes t
		INNER JOIN AutoWho.CaptureTimes targ
	...
	BEGIN
		...
		EXEC AutoWho.LogEvent @ProcID=@@PROCID, @EventCode=0, @TraceID=NULL, @Location=N'ResolveLtchWdur', @Message=@errormsg;
	END
END TRY
BEGIN CATCH
