...
GO
CREATE PROCEDURE [AutoWho].[ResolveNodeStatusInfo] 
/*   
	...
	FILE NAME: AutoWho.ResolveNodeStatusInfo.StoredProcedure.sql

	PROCEDURE NAME: AutoWho.ResolveNodeStatusInfo
...
To Execute
------------------------
EXEC AutoWho.ResolveNodeStatusInfo @CollectionInitiatorID = 255, @FirstCaptureTimeUTC='2017-07-24 04:00', @LastCaptureTimeUTC='2017-07-24 06:00'
*/
(
	@CollectionInitiatorID	TINYINT,
	@FirstCaptureTimeUTC	DATETIME,	--This proc ASSUMES that these are valid UTC capture times in AutoWho.CaptureTimes
...
BEGIN TRY
	...
	BEGIN
		...
		BEGIN
			EXEC AutoWho.LogEvent @ProcID=@@PROCID, @EventCode=0, @TraceID=NULL, @Location=N'No Unprocessed NodeStatus', @Message='No unprocessed NodeStatus rows to process';
		END
		RETURN 0;
	END
	...
	BEGIN
		...
		EXEC AutoWho.LogEvent @ProcID=@@PROCID, @EventCode=0, @TraceID=NULL, @Location=N'ResolveNSdur', @Message=@errormsg;
	END
END TRY
BEGIN CATCH
