...
/*   
	...
	PURPOSE: This is the heart of AutoWho. This proc collects data from a wide array of session-focused DMVs and stores 
		...
		The AutoWho.Options table's extended properiets includes meanings for most of the parameters to this procedure, and 
...
/* Control Flow Summary
	...
	14. For user spids with calc__return_to_user > 0, we join the #sar table to the SQL statement store (CoreXR.SQLStmtStore)
		...
		CoreXR.SQLStmtStore before. This can potentially avoid the cost of resolving lots of sql_handle/offset combos
...
*/
	...
	*/
	...
	FROM #sessions_and_requests targ 
		INNER loop JOIN CoreXR.SQLStmtStore sss
	...
	BEGIN
		...
		FROM #sessions_and_requests targ 
			INNER loop JOIN CoreXR.SQLBatchStore sbs
	...
	*/
	...
	BEGIN
		...
		MERGE CoreXR.SQLStmtStore perm
			USING #t__stmt t
		...
		FROM #sessions_and_requests sar
			INNER hash JOIN CoreXR.SQLStmtStore sss
	...
	BEGIN
		...
		MERGE CoreXR.SQLBatchStore perm
			USING #t__batch t
				ON perm.sql_handle = t.sql_handle
		WHEN MATCHED THEN UPDATE
	...
	BEGIN
		...
		--WHERE NOT EXISTS (
		--	SELECT *
		--	FROM CoreXR.QueryPlanBatchStore bs2
		...
		MERGE CoreXR.QueryPlanBatchStore perm
			USING #t__batchqp t
	...
	BEGIN
		...
		--WHERE NOT EXISTS (
		--	SELECT * 
		--	FROM CoreXR.QueryPlanStmtStore qps
		...
		MERGE CoreXR.QueryPlanStmtStore perm
			USING #t__stmtqp t
		...
		*/
		...
		FROM #t__stmtqp t
			INNER JOIN CoreXR.QueryPlanStmtStore qps
