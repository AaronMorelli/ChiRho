...
/*   
	...
	FILE NAME: AutoWho.PopulateCaptureSummary.StoredProcedure.sql

	PROCEDURE NAME: AutoWho.PopulateCaptureSummary
	...
	PURPOSE: Pulls data from the AutoWho base tables and aggregates various characteristics of the data into a summary row per SPIDCaptureTime/UTCCaptureTime. 
			...
			NOTE: The delta logic has been moved to a sub-proc and is now driven by a new "CaptureSummaryDeltaPopulated" flag in AutoWho.CaptureTimes.
			These changes were made for ease of accuracy and understandability of the code.

To Execute
------------------------
EXEC AutoWho.PopulateCaptureSummary @CollectionInitiatorID=255, @StartTime='2016-04-25 08:00', @EndTime='2016-04-25 09:00'
*/
(
...
BEGIN
	...
	BEGIN TRY
		...
		IF NOT EXISTS (SELECT * FROM AutoWho.CaptureTimes ct WHERE ct.SPIDCaptureTime BETWEEN @StartTime AND @EndTime)
		BEGIN
			RETURN 1;
		END
		...
		/* Conceptually what we want to do is aggregate info from SAR and TAW down into a single capture time, so that for each capture time,
			...
			3. We need to correctly handle cases where the time "falls back" due to Daylight Savings Time. Thus, when the clock falls back
				...
				that delta logic compares to the "true" previous capture, and that we don't accidentally allow certain captures to be
				"skipped".
		...
		*/
		...
		BEGIN
			...
			EXEC AutoWho.PopulateCaptureSummaryDelta @CollectionInitiatorID=@CollectionInitiatorID, @StartTime=@StartTime, @EndTime=@EndTime;
			RETURN 0;
		END
		...
		FROM #CTTP t
			INNER loop JOIN AutoWho.CaptureTimes targ WITH (ROWLOCK)	--hints to avoid conflicting with the Collector
