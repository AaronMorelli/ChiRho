...
GO
CREATE PROCEDURE [AutoWho].[PresentSessionViewer] 
/*   
	...
	FILE NAME: AutoWho.PresentSessionViewer.StoredProcedure.sql

	PROCEDURE NAME: AutoWho.PresentSessionViewer
...
FROM AutoWho.DimWaitType dwt INNER JOIN AutoWho.TasksAndWaits taw WITH (NOLOCK) ON taw.FKDimWaitType = dwt.DimWaitTypeID' + NCHAR(10) + NCHAR(13) + N'--?>')
	...
	BEGIN
		...
		SELECT 
			...
			qpbs.plan_handle,
			qpbs.query_plan
		FROM CoreXR.QueryPlanBatchStore qpbs
		WHERE EXISTS (
			SELECT *
			FROM #DistinctFKs fk
				...
				BEGIN
					...
					BEGIN CATCH
						...
						CASE WHEN ERROR_NUMBER() = 6335 AND @PKSQLStmtStoreID IS NOT NULL THEN 
							...
							SELECT query_plan
							INTO dbo.largeQPbcpout
							FROM CoreXR.QueryPlanBatchStore q
								...
								CASE WHEN bg.request_id = @lv__nullsmallint
										THEN ( 
											...
											(SELECT 1 as col1) dummy1 
												...
												FROM CoreXR.InputBufferStore b
